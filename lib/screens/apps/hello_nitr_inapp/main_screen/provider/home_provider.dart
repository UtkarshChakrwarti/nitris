...
class HomeProvider {
  ...
  static Future<List<User>> fetchContacts(
    ...
    try {
      _logger.info(
          'Fetching contacts with offset: $offset, limit: $limit, filter: $filter, isAscending: $isAscending');
  ...
  static Future<List<User>> searchUsers(
    ...
    try {
      _logger.info(
          'Searching users with offset: $offset, limit: $limit, query: $query');
  ...
  static Future<List<User>> searchUsersFiltered(
      int offset, int limit, String query, String filter) async {
    try {
      _logger.info(
          'Searching users with offset: $offset, limit: $limit, query: $query, filter: $filter');
      final users = await LocalStorageService.searchUsersFiltered(
          offset, limit, query, filter);
      _logger.info('Found ${users.length} users.');
      return users;
    } catch (error) {
      _logger.severe('Failed to search users: $error');
  ...
  static Future<List<User>> searchUsersByDepartment(
    ...
    try {
      _logger.info(
          'Searching users by department with offset: $offset, limit: $limit, query: $query, department: $department');
